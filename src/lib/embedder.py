...
class BatchEmbedder:
    ...
    def __init__(self, model: str = None, batch_size: int = 10, timeout: int = 30):
        ...
        logger.debug(f"Initialized BatchEmbedder with model={self.model}, batch_size={self.batch_size}")
    ...
    async def embed_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        if len(filtered_texts) < len(texts):
            logger.debug(f"Filtered out {len(texts) - len(filtered_texts)} empty texts before embedding")
